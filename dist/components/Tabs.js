...
const Tabs = () => {
    ...
    return (<section className="max-w-lg mx-auto space-y-4">
			{/* TAB HEADER */}
			<div className="grid rounded-xl bg-base-200 p-1" style={{ gridTemplateColumns: `repeat(${tabs.length}, 1fr)` }}>
				{tabs.map((tab) => (<a key={tab.id} role="tab" className={`flex cursor-pointer select-none items-center justify-center gap-2 rounded-lg py-2 text-sm font-medium ${activeTab === tab.id
                ? "animate-opacity bg-base-100 shadow"
                : "text-base-content/75"}`} onClick={() => setActiveTab(tab.id)}>
						{tab.icon}
						{tab.title}
					</a>))}
			</div>
